...
The most attention is put on Time & Space complexity of the model, thus scientific computing libraries such as Numpy and Scipy are used in nearly all operations along with Numba libray which boosts the computation speed by parallelizing the numpy-heavy functions with JIT(just-in-time compilation).  
