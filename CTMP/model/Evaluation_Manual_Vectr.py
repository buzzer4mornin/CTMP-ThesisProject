...
parser.add_argument("--sample_test", default=2000, type=int, help="Size of test set")
parser.add_argument("--TOP_M_start", default=10, type=int, help="Start of Top-M recommendation")
parser.add_argument("--TOP_M_end", default=13, type=int, help="End of Top-M recommendation")
...
class Evaluation:
    ...
    def predict_out_of_matrix(self, user_id):
        ...
        predicted_top_M_TEST = np.setdiff1d(sorted_ratings, self.rating_GroupForUser_TRAIN[user_id], assume_unique=True)[:self.top_m]
        predicted_top_M_TRAIN = sorted_ratings[:self.top_m]
        ...
        self.precisions_out_of_matrix_TRAIN += (top_m_correct_TRAIN / self.top_m)
        self.recalls_out_of_matrix_TEST += (top_m_correct_TEST / len(self.rating_GroupForUser_TEST[user_id]))
        self.precisions_out_of_matrix_TEST += (top_m_correct_TEST / self.top_m)

    def avg_recall_precision(self) -> None:
        self.test_set = sorted(self.test_set)
        self.whole_rating = np.dot(self.shp[self.test_set] / self.rte[self.test_set], self.mu.T)
        for top in range(args.TOP_M_start, args.TOP_M_end):
            ...
            if args.pred_type == "both":
                ...
                for i in range(len(self.test_set)):
                    self.predict_in_matrix(self.test_set[i], top, self.whole_rating[i])
                ...
                # for i in range(len(self.test_set)):
                #     self.predict_out_of_matrix(self.test_set[i])

                list(map(self.predict_out_of_matrix, self.test_set))
                # self.recalls_out_of_matrix_TRAIN += x1
                # self.precisions_out_of_matrix_TRAIN += x2
                # self.recalls_out_of_matrix_TEST += x3
                # self.precisions_out_of_matrix_TEST += x4
    ...
    def plot(self) -> None:
        ...
        # plt.savefig('../input-data/eval/EXAMPLE-30.png')
        plt.show()
