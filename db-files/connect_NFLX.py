...
try:
    ...
    '''cur.execute("select USERID from A_FUSERS")
    db.commit()
    df = pd.DataFrame(cur.fetchall())
    df.columns = ["USERID"]
    df.to_pickle(currdir + '/df_user_NFLX')
    print("Table Created successful")'''

    # -- 2nd Query --  [Get RATING table] [UNCOMMENT to run]
    cur.execute("select USERID, MOVIEID, RATING from A_FRATINGS")
    db.commit()
    df = pd.DataFrame(cur.fetchall())
    df.columns = ["USERID", "MOVIEID", "RATING"]
    df.to_pickle(currdir + '/df_rating_NFLX')
    print("Table Created successful")

    # -- 3rd Query --  [Get MOVIE table] (parse XML plot from IMDB) [UNCOMMENT to run]
    '''cur.execute(
        "SELECT e.TT, e.XML.getClobval() AS coXML, A_FMOVIES.MOVIEID  FROM IMDB e inner join A_FMOVIES on e.TT = A_FMOVIES.TT")
    db.commit()
    df = cur.fetchall()
    ...
    df.to_pickle(currdir + '/df_movie_NFLX')
