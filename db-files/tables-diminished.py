...
# [1, 47, 119, 358, 521] - movies with these indeces are selected for DIMINISHED datasets
movie_dim = movie_df.iloc[[1, 47, 119, 358, 521], :]
movie_dim.loc[:, "new_MOVIEID"] = range(0, 5)
...
user_dim = user_df.iloc[[0, 1, 2, 3, 4], :]
user_dim.to_pickle(currdir + '/df_user_DIMINISHED')


# ------- Diminished Rating -------
rating_df = pd.read_pickle("df_rating_UPDATED")
users = [i for i in list(range(0, 5)) for _ in range(5)]
movies = [i for _ in range(5) for i in list(range(0, 5))]
